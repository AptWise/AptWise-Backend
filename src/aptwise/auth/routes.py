...
from .models import UserCreate, UserLogin, UserResponse, LinkedInUserProfile, LinkedInAuthRequest, GitHubUserProfile, GitHubAuthRequest
...
async def get_current_user_info(current_user: Optional[str] =
    ...
    if current_user is None:        raise HTTPException(
    ...
    if db_user:
        return {
            ...
            "is_github_connected": db_user.get("is_github_connected", False)
        }
...
async def linkedin_redirect_callback(code: str = None, state: str = None, error: str = None, error_description: str = None):
...
async def github_redirect_callback(code: str = None, state: str = None, error: str = None, error_description: str = None):
...
async def github_callback(auth_request: GitHubAuthRequest, response: Response):
    ...
    try:
        ...
        github_profile = await github_oauth_service.get_user_info(access_token)
        ...
        if existing_user:
            ...
            success = update_user_github_connection(github_profile["email"], github_data)
            if not success:
                raise HTTPException(
                    status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
        ...
        # Create JWT token for the user
        jwt_token = create_access_token(data={"sub": github_profile["email"]})
        set_access_cookies(response, jwt_token)
...
async def connect_github(auth_request: GitHubAuthRequest, 
                        current_user: Optional[str] = Depends(get_current_user)):
...
async def disconnect_github(current_user: Optional[str] = Depends(get_current_user)):
...
async def get_github_profile(current_user: Optional[str] = Depends(get_current_user)):
...
async def debug_github_config():
    ...
    from ..config.github_config import GITHUB_CLIENT_ID, GITHUB_CLIENT_SECRET, GITHUB_REDIRECT_URI
    ...
    return {
        ...
        "service_configured": github_oauth_service.configured if hasattr(github_oauth_service, 'configured') else "Unknown"
    }
