...
class GitHubOAuthService:
    ...
    def __init__(self):
        ...
        if not self.configured:
            print("WARNING: GitHub OAuth credentials not configured. "
                  "GitHub features will be disabled.")
    ...
    async def get_user_profile(self, access_token: str) -> Dict[str, Any]:
        ...
        async with httpx.AsyncClient() as client:
            # Get user profile
            profile_response = await client.get(GITHUB_USER_URL,
                                                headers=headers)
            ...
            if not email:
                ...
                email_response = await client.get(GITHUB_USER_EMAIL_URL,
                                                  headers=headers)
                if email_response.status_code == 200:
                    emails = email_response.json()
                    # Find primary email or first verified email
                    for email_info in emails:
                        if (email_info.get("primary", False) and
                                email_info.get("verified", False)):
