...
class LinkedInOAuthService:
    ...
    def __init__(self):
        ...
        if not self.configured:
            print("WARNING: LinkedIn OAuth credentials not configured. "
                  "LinkedIn features will be disabled.")
    ...
    async def get_user_profile(self, access_token: str) -> Dict[str, Any]:
        ...
        async with httpx.AsyncClient() as client:
            # Use the newer userinfo endpoint which works with
            ...
            return {
                ...
                "full_name": userinfo_data.get(
                    "name",
                    f"{userinfo_data.get('given_name', '')} "
                    f"{userinfo_data.get('family_name', '')}"
                ).strip()
            }
